...
try {
    ...
    $html = generarHTMLTicketPDF($venta, $empresa, $numeroStr, $fechaStr, $nombreCajero, $hasFinanciacion);

    // Intentar generar PDF con html2pdf (composer)
    ...
    if (file_exists(__DIR__ . '/../vendor/autoload.php')) {
        try {
            ...
            // Continuar sin PDF if html2pdf falla
        }
    }

    // Fallback: Si html2pdf no funciona, devolver HTML para que el navegador lo maneje
...
function generarHTMLTicketPDF($venta, $empresa, $numeroStr, $fechaStr, $nombreCajero, $hasFinanciacion) {
    $html = "<!DOCTYPE html>
<html>
<head>
    <meta charset='UTF-8'>
...
<body>
    ...
    foreach ($venta['lineas'] as $linea) {
        ...
        if (!empty($linea['numeros_serie'])) {
            $html .= "<br/><small style='color: #999;'>S/N: " . htmlspecialchars($linea['numeros_serie']) . "</small>";
        }
    ...
    $html .= "
        <div class='footer'>
            ...
            <p>Conserve este ticket para devoluciones dentro de 14 días</p>
